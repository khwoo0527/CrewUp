...
**상태:** ✅ 완료 (자가 리뷰 이슈 5건 → backlog 🔴 Critical 등록, Sprint 2 마무리에서 fix 예정)
...
| Task | 설명 | 대상 | skill |
|------|------|------|-------|
| Task 0 | 테스트 자동화 기반 구축 | jest + testing-library 설치/설정 | -- |
...
| Task 6 | 통합 검증 + 테스트 작성 + 더미 데이터 제거 | 전체 | -- |

> "Phase 2를 팀으로 실행해줘"라고 요청하면 Task 3, Task 4를 병렬로 구현합니다.

---

### Task 0: 테스트 자동화 기반 구축

**Files:**
- Modify: `package.json` (jest, testing-library 의존성 추가)
- Create: `jest.config.js` (또는 `jest.config.ts`)
- Create: `jest.setup.ts` (테스트 환경 설정)
- Create: `src/__tests__/setup.test.ts` (설정 검증용 스모크 테스트)

**Step 1: 패키지 설치**
- jest + jest-expo (Expo 전용 Jest preset)
- @testing-library/react-native (컴포넌트 테스트)
- @testing-library/jest-native (커스텀 매처)
- @types/jest (타입)

**Step 2: Jest 설정**
- `jest.config.js`: preset `jest-expo`, transformIgnorePatterns 설정 (Expo/RN 패키지 트랜스파일)
- `jest.setup.ts`: 모킹 설정 (expo-router, supabase, async-storage 등)
- `package.json`에 `"test": "jest"` 스크립트 추가

**Step 3: 스모크 테스트**
- 간단한 테스트 1개 작성하여 Jest 설정이 정상 동작하는지 확인
- `npx jest` 실행 → 통과 확인

**Step 4: 커밋**
```
git add jest.config.js jest.setup.ts package.json src/__tests__/
git commit -m "chore(sprint2): 테스트 자동화 기반 구축 — jest + testing-library 설정"
```

**완료 기준:**
- [x] `npx jest` 실행 시 테스트 통과
- [x] Expo/RN 패키지 트랜스파일 정상 (transformIgnorePatterns)
- [x] 모킹 설정 완료 (supabase, expo-router 등)
- [x] `npx tsc --noEmit` 통과
...
**완료 기준:**
- [x] 4개 테이블 생성 완료 (Supabase 대시보드에서 확인)
- [x] 모든 테이블에 RLS 활성화 + 정책 적용
- [x] sport_categories에 '테니스' 시드 데이터 존재
- [x] `src/services/supabase/types.ts`에 새 테이블 타입 포함
- [x] `npx tsc --noEmit` 통과
...
**완료 기준:**
- [x] `src/features/club/types.ts`에 DB 타입에서 추출한 타입 정의
- [x] `src/features/club/constants.ts`에 club_type, role, status 상수
- [x] `src/services/supabase/database.ts`에 CRUD 쿼리 함수 (fetchClubs, fetchClubById, searchClubs, createClub, createClubMember, fetchMyClubs)
- [x] 모든 쿼리 함수에 에러 처리 포함
- [x] `npx tsc --noEmit` 통과
...
**완료 기준:**
- [x] `/club/create` 경로에서 동호회 생성 폼 표시
- [x] zod 검증 동작 (이름 2~30자 등)
- [x] 생성 시 clubs + club_members(owner) 동시 등록
- [x] 생성 완료 후 캐시 무효화 + 화면 이동
- [x] 모바일/데스크톱 반응형 정상
- [x] `npx tsc --noEmit` 통과
- ⚠️ 자가 리뷰: create.tsx에서 supabase 직접 호출 → 훅으로 분리 필요 (backlog 🔴)
...
**완료 기준:**
- [x] 홈 화면에서 Supabase 실제 데이터로 동호회 목록 표시
- [x] 이름 검색 동작 (디바운스 적용)
- [x] 카드 탭 → 동호회 상세 페이지 이동
- [x] 상세 페이지에 동호회 정보 표시 (이름, 소개, 지역, 멤버 수 등)
- [x] 로딩/에러/빈 상태 처리 (로딩 ✅, 빈 상태 ✅, 에러 ⚠️ UI 미흡 → backlog)
- [x] 반응형 그리드 (모바일 1열, 데스크톱 3열)
- [x] `npx tsc --noEmit` 통과
...
**완료 기준:**
- [x] 가입된 동호회가 없으면 EmptyState 표시 (CTA "동호회 찾기")
- [x] 가입된 동호회가 있으면 ClubCard 목록 표시
- [x] 카드 탭 → 동호회 상세 이동
- [x] "동호회 만들기" 버튼 → `/club/create` 이동
- [x] `npx tsc --noEmit` 통과

---

### Task 6: 통합 검증 + 테스트 작성 + 정리

**Files:**
- Modify: `src/features/club/index.ts` (최종 re-export 정리)
- Modify: `app/_layout.tsx` (club 라우트 등록 확인)
- Create: `src/features/club/__tests__/constants.test.ts`
- Create: `src/features/club/__tests__/ClubCard.test.tsx`
- Create: `src/services/supabase/__tests__/database.test.ts`

**Step 1: 단위 테스트 작성**
- 상수 테스트: club 상수값 검증 (CLUB_TYPES, MEMBER_ROLES 등)
- 컴포넌트 테스트: ClubCard 렌더링 + onPress 콜백 호출 검증
- 쿼리 함수 테스트: supabase 모킹 후 fetchClubs, createClub 등 에러 처리 검증
- `npx jest` 실행 → 전체 통과 확인
...
```
git add -A
git commit -m "feat(sprint2): 동호회 CRUD 통합 검증 + 단위 테스트 + 정리"
```

**완료 기준:**
- [x] 전체 플로우 (생성 → 목록 → 상세 → 내 동호회) 정상
- [x] 더미 데이터 완전 제거
- [x] 빌드 성공 (`npx tsc --noEmit`)
- [x] 테스트 통과 (`npx jest`)
- [x] 로딩/에러/빈 상태 모두 처리
- [x] 반응형 정상 (모바일 + 데스크톱)
...
| Vercel 배포 | git push → Vercel 자동 빌드 → 사이트 접속 | 동호회 CRUD 모바일에서 정상 동작 |
