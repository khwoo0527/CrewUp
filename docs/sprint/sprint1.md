...
**완료 기준:**
- [ ] `npx supabase start` 정상 실행
- [ ] profiles 테이블 생성 + RLS 정책 활성화
- [ ] `supabase gen types` → `types.ts` 생성
- [ ] 클라이언트에 `Database` 제네릭 적용
- [ ] `npx tsc --noEmit` 통과
...
**완료 기준:**
- [ ] `useAuth` 훅이 세션 상태를 정확히 반영
- [ ] `signInWithKakao`, `signInWithNaver`, `signOut` 함수 정의
- [ ] `onAuthStateChange` cleanup 처리
- [ ] `isOnboarded` (닉네임 존재 여부) 정상 판단
- [ ] `npx tsc --noEmit` 통과
...
**완료 기준:**
- [ ] 앱 시작 시 로그인 화면 표시 (미로그인 상태)
- [ ] 카카오/네이버 로그인 버튼 표시 (공식 색상)
- [ ] 온보딩 화면에 닉네임 입력 + 지역 선택
- [ ] 인증 가드: 미로그인 → 로그인, 프로필 미완성 → 온보딩
- [ ] `npx tsc --noEmit` 통과
- [ ] 모바일 + 데스크톱 반응형 정상
...
**완료 기준:**
- [ ] 프로필 탭에 닉네임/지역/가입일 표시
- [ ] 프로필 수정 → DB 반영
- [ ] 로그아웃 → 세션 삭제 + 로그인 화면 이동
- [ ] 앱 재시작 시 세션 유지 (자동 로그인)
- [ ] 전체 인증 플로우 (로그인 → 온보딩 → 메인 → 프로필 → 로그아웃) 정상
- [ ] `npx tsc --noEmit` 통과
...
| 로그인 화면 | 앱 시작 시 표시 | ✅ 카카오/네이버 버튼 |
| 소셜 로그인 | 카카오/네이버 로그인 | ✅ 세션 생성 + profiles 자동 생성 |
