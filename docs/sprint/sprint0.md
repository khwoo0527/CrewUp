...
**완료 기준:**
- [ ] `npx expo start --web` 정상 실행
- [ ] `npx tsc --noEmit` 타입 에러 없음
- [ ] `npx expo lint` 린트 통과
- [ ] 기존 파일 (CLAUDE.md, ROADMAP.md, .claude/, docs/) 보존 확인
- [ ] HTML 목업 7개 → `docs/mockup/` 이동 완료
- [ ] `.env.local` 생성 + `.gitignore` 반영
- [ ] `src/shared/constants/` 상수 파일 생성 (app.ts, limits.ts)
...
**완료 기준:**
- [ ] NativeWind 클래스 웹 + 모바일에서 동작 (색상, 폰트, 정렬)
- [ ] 테마 색상 (primary #2e7d32 등) tailwind.config.js에 반영
- [ ] 반응형 브레이크포인트 (md:, lg:) 동작 확인
- [ ] `npx supabase start` → 로컬 Supabase 정상 실행
- [ ] 앱에서 Supabase 연결 성공 (콘솔 로그)
- [ ] 환경 변수 타입 안전 접근 (`src/config/env.ts`)
...
**완료 기준:**
- [ ] Button: 4 variant × 3 size 조합 정상 렌더링
- [ ] Button: isLoading 시 스피너 표시
- [ ] Card: onPress 있을 때 Pressable, 없을 때 View
- [ ] Badge: 5 variant 정상 렌더링
- [ ] 디자인 토큰이 tailwind.config.js와 동기화
- [ ] 모든 컴포넌트 NativeWind 클래스 기반
- [ ] `npx tsc --noEmit` 통과
...
**완료 기준:**
- [ ] 4개 탭 (홈, 내 동호회, 알림, 프로필) 전환 정상
- [ ] 각 탭에 EmptyState 플레이스홀더 표시
- [ ] Container: 반응형 max-width 동작
- [ ] LoadingSpinner: 중앙 스피너 표시
- [ ] EmptyState: 아이콘 + 제목 + 설명 + CTA 버튼
- [ ] ResponsiveGrid: 모바일 1열, 태블릿 2열, 데스크톱 3열
- [ ] useDeviceType: isMobile/isTablet/isDesktop 정상
- [ ] React Query Provider 정상 동작
- [ ] home.tsx에 더미 카드 6개 반응형 표시
- [ ] `npx expo start --web` 정상
- [ ] `npx tsc --noEmit` 통과
- [ ] `npx expo lint` 통과
